...
async def prog(ctx):
    ...
    print(f"[{ctx.myid}] Greater Result = ", res)


    K3 = 8000
    k3 = ctx.Share(K3) + ctx.preproc.get_zero(ctx)
    # print(f"[{ctx.myid}] k3 = ", k3)
    res = await (k1 < k3).open()
    print(f"[{ctx.myid}] Less Result = ", res)
...
async def tutorial_1():
    ...
    pp.generate_shared_bits(10000, n, t)
